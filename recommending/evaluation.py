...
def preprocess_data(data):
    ...
    data['Year-Of-Publication'].fillna(data['Year-Of-Publication'].median(), inplace=True)
...
train_matrix = csr_matrix((train_data['Book-Rating'], (train_data['User-ID'], train_data['book_id'])), shape=(n_users, n_items))
test_matrix = csr_matrix((test_data['Book-Rating'], (test_data['User-ID'], test_data['book_id'])), shape=(n_users, n_items))
...
# dataset = TensorDataset(user_ids, book_ids, rates, ages, locations, authors, years, publishers)
...
def recommend_books(user_id, model, num_recommendations=15):
    user_index = torch.tensor([user_id_mapping[user_id]]).long()  # Ensure user_index is a Tensor
    ...
    recommended_book_indices = sorted(scores, key=scores.get, reverse=True)[:num_recommendations]
    recommended_books = [inverse_book_id_mapping[idx] for idx in recommended_book_indices]
...
def evaluate_model2(test_dataloader, model, k=15):
    ...
    for batch in test_dataloader:
        ...
        for i, user_id in enumerate(user_ids):
            user_index = torch.tensor([user_id_mapping[user_id.item()]]).long()  # Ensure user_index is a Tensor
            ...
            recommended_book_embeddings = np.array([book_embeddings[book_id_mapping[book]] for book in recommended_books])
...
def test_one_user(user_id, test_dataloader, model, k=10):
    ...
    for batch in test_dataloader:
        ...
        for i, uid in enumerate(user_ids):
            if uid.item() == user_id:
                user_data = {
                    'user_index': torch.tensor([user_id_mapping[uid.item()]]).long(),  # Ensure user_index is a Tensor
                    'user_location': locations[i].unsqueeze(0),
                    'user_age': ages[i].unsqueeze(0),
